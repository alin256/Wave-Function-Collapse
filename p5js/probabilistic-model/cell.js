...
class Cell {
  ...
  updateProbabilities() {
    ...
    // make incomplete update
    for (let i = 0; i < this.probabilities.length; i++) {
      let targetProbability = this.probabilities[i] * this.likelihoods[i];
      this.probabilities[i] = this.probabilities[i] * (1 - alpha) + targetProbability * alpha;
    }
    this.scaleProbabilities();
  }

  tryCollapse(collapseConst) {
    for (let i = 0; i < this.probabilities.length; i++) {
      if (this.probabilities[i] > collapseConst) {
        this.collapsed = true;
        this.needsRedraw = true;
        break;
      }
